...
{
    /// <summary>
    /// <see cref="int"/> and <see cref="float"/> implementation of <see cref="IFTwoArgumentEventListener{T0, T1}"/>
    /// </summary>
    /// <inheritdoc cref="IFTwoArgumentEventListener{T0, T1}"/>
    /// <inheritdoc cref="IFSEventListener"/>
    public class FSIntFloatEventListener : IFTwoArgumentEventListener<int, float>
    {
        /// <summary>
        /// The FSEventSO to subscribe to of type <see cref="FSIntFloatEventSO"/>
        /// </summary>
        [Tooltip("The <int float> event to listen")]
        public FSIntFloatEventSO fsIntFloatEventSo;
        ...
        [SerializeField] public FSIntFloatEvent listeners = new FSIntFloatEvent();
        ...
        private void AutoAdd()
        {
            ...
            var eventName = fsIntFloatEventSo.name.Replace(" ", "");
            ...
            foreach (var monoBehaviour in allBehaviours)
            {
                ...
                foreach (var methodInfo in methodName)
                {
                    ...
                    var action = methodInfo.CreateDelegate(typeof(UnityAction), monoBehaviour) as UnityAction<int, float>;
        ...
        {
            fsIntFloatEventSo.fsTwoArgumentAction += Invoke;
        }
        ...
        protected override void Invoke(int valueOne, float valueTwo)
        {
